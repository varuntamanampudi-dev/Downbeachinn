  ...
  return (
    <div style={{ flex: '1 1 260px', minWidth: 0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', marginBottom: '0.35rem' }}>
        {DAY_LABELS.map((d) => (
          <div key={d} style={{ textAlign: 'center', fontSize: '0.72rem', fontWeight: 600, color: '#9ca3af', padding: '0.2rem 0' }}>
            {d}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', rowGap: '2px' }}>
        {cells.map((day, i) => {
          ...
          const isHoverPreview = iso === hoveredDate && checkIn && !checkOut && hoveredDate > checkIn;

          const bgColor = isEndpoint || isHoverPreview
            ? 'var(--color-teal)'
            : inRange
            ? 'rgba(13,148,136,0.12)'
            : 'transparent';

          const textColor = isEndpoint || isHoverPreview
            ? 'white'
            : isPast
            ? '#d1d5db'
            : isToday
          ...
          // Round only the endpoints; range interior stays square for connected look
          const borderRadius = isEndpoint || isHoverPreview || (!inRange && !isHoverPreview)
            ? '50%'
            : '0';
...
export default function HeroBookingWidget() {
  ...
  return (
    <div ref={wrapperRef} style={{ position: 'relative', maxWidth: '720px', width: '100%' }}>
      ...
      <div style={{
        background: 'white',
        borderRadius: calendarOpen ? 'var(--radius-xl) var(--radius-xl) 0 0' : 'var(--radius-xl)',
        padding: '0.5rem',
        display: 'flex',
        flexWrap: 'wrap',
        ...
        <div
          ...
          style={{
            ...
            background: calendarOpen && selecting === 'in' ? '#f0fdfa' : 'transparent',
        ...
        <div
          ...
          style={{
            ...
            cursor: 'pointer',
            borderRadius: 'var(--radius-lg)',
            background: calendarOpen && selecting === 'out' ? '#f0fdfa' : 'transparent',
            transition: 'background 0.15s',
          }}
        >
          <div style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-teal)', marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
        ...
        <div style={{ flex: '1 1 140px', padding: '0.5rem' }}>
          <button
            ...
            style={{
              ...
              background: ready ? 'var(--color-teal)' : '#94a3b8',
              color: 'white', border: 'none', borderRadius: 'var(--radius-lg)',
              fontWeight: 700, fontSize: '0.95rem', cursor: ready ? 'pointer' : 'not-allowed',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              gap: '0.4rem', transition: 'background 0.2s', letterSpacing: '0.02em',
            }}
            onMouseEnter={(e) => { if (ready) e.currentTarget.style.background = '#14b8a6'; }}
            onMouseLeave={(e) => { if (ready) e.currentTarget.style.background = ready ? 'var(--color-teal)' : '#94a3b8'; }}
          >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            <div style={{ fontSize: '0.85rem', color: '#6b7280', fontWeight: 500 }}>
              {selecting === 'in'
                ...
                : checkIn
                ? `Check-in: ${formatDisplay(checkIn)} — now select check-out`
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
            ...
            <button
              ...
              disabled={!canPrev}
              style={{
                flexShrink: 0, width: '34px', height: '34px', marginTop: '0.2rem',
                background: 'none', border: '1px solid #e5e7eb', borderRadius: '50%',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                cursor: canPrev ? 'pointer' : 'not-allowed', opacity: canPrev ? 1 : 0.3,
              }}
            >
            ...
            <div style={{ flex: 1, display: 'flex', gap: '2.5rem', flexWrap: 'wrap' }}>
              <CalendarMonth
                ...
                hoveredDate={hoveredDate} today={today}
                onSelect={handleSelect} onHover={setHoveredDate}
              />
              <CalendarMonth
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', marginTop: '1.25rem', paddingTop: '1rem', borderTop: '1px solid #f3f4f6' }}>
            <button
              onClick={clearDates}
              style={{ padding: '0.5rem 1.1rem', background: 'none', border: '1px solid #e5e7eb', borderRadius: 'var(--radius-md)', fontSize: '0.85rem', fontWeight: 600, color: '#374151', cursor: 'pointer' }}
            >
              Clear
            </button>
            <button
              ...
              disabled={!ready}
              style={{ padding: '0.5rem 1.25rem', background: ready ? 'var(--color-teal)' : '#94a3b8', color: 'white', border: 'none', borderRadius: 'var(--radius-md)', fontSize: '0.85rem', fontWeight: 700, cursor: ready ? 'pointer' : 'not-allowed' }}
            >
              Done
            </button>
          </div>
