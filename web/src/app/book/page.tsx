    ...
    <>
      ...
      <section style={{ background: 'linear-gradient(135deg, var(--color-sky-50) 0%, var(--color-sky-100) 100%)', padding: '4rem 0 3rem', borderBottom: '1px solid var(--color-sky-200)' }}>
        <div className="section-container">
          <p style={{ color: 'var(--color-sky-600)', fontWeight: 600, fontSize: '0.85rem', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Reservations</p>
          <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3rem)', fontWeight: 800, color: 'var(--color-sky-900)', letterSpacing: '-0.02em', marginBottom: '0.75rem' }}>Book Your Stay</h1>
      ...
      <section style={{ padding: '3rem 0 5rem', background: 'var(--color-surface)' }}>
        <div className="section-container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '3rem', alignItems: 'start' }}>
            <BookingForm roomTypes={roomTypes} taxRate={taxRate} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div className="glass-card" style={{ padding: '1.75rem' }}>
                ...
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '0.7rem' }}>
                  {['Lowest guaranteed rate', 'Free cancellation (48h notice)', 'Instant confirmation email', 'No third-party booking fees', 'Direct contact with our team'].map((item) => (
              ...
              <div className="glass-card" style={{ padding: '1.75rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <a href={`tel:+1${MOTEL.phonePlain}`} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', fontSize: '0.875rem', color: 'var(--color-sky-700)', fontWeight: 600 }}>
                    <span>📞</span> {MOTEL.phone}
                  </a>
                  <a href={`mailto:${MOTEL.email}`} style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', fontSize: '0.875rem', color: 'var(--color-sky-700)', fontWeight: 600 }}>
                    <span>✉️</span> {MOTEL.email}
                  </a>
                </div>
              </div>

              {/* Tax note */}
              <div style={{ background: 'var(--color-sky-50)', border: '1px solid var(--color-sky-200)', borderRadius: 'var(--radius-md)', padding: '1rem 1.25rem' }}>
                <p style={{ fontSize: '0.8rem', color: 'var(--color-sky-700)', fontWeight: 600, marginBottom: '0.25rem' }}>About Pricing</p>
                <p style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)', lineHeight: 1.5 }}>
                  Nightly rates start at $65.00. NJ hotel/motel tax of {taxRate}% is added at checkout. Total shown includes all taxes — no hidden fees.
                </p>
              </div>
            </div>
          </div>
